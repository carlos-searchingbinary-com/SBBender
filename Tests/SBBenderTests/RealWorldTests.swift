...
struct RealWorldTests {
    ...
    // MARK: - Real Tokenization + Custom Tool in Sequential Pipeline

    @Test("Sequential pipeline: tokenize text then count via custom tool")
    func testSequentialRealPipeline() async throws {
        // Step 1: Tokenize text into sentences using real Apple NLP
        let tokenCall = ToolCall(
            id: "tc-tok",
            name: "tokenize",
            arguments: #"{"input": "The quick brown fox jumps. Over the lazy dog. Then it rests.", "unit": "sentence"}"#
        )
        let tokenAgent = Agent(
            configuration: AgentConfiguration(name: "Tokenizer"),
            model: MockProvider(
                responses: ["Tokenized: 3 sentences found."],
                toolCallResponses: [([tokenCall], "")]
            ),
            nativeTools: [TokenizationSkill()]
        )

        // Step 2: Take the output and verify word count with a custom tool
        let wordCountCall = ToolCall(
            id: "tc-wc",
            name: "wordCount",
            arguments: #"{"text": "The quick brown fox jumps. Over the lazy dog. Then it rests."}"#
        )
        let counterAgent = Agent(
            configuration: AgentConfiguration(name: "Counter"),
            model: MockProvider(
                responses: ["Word count: 14 words."],
                toolCallResponses: [([wordCountCall], "")]
            ),
            tools: [
                Tool(name: "wordCount", description: "Count words in text", parameters: JSONSchema(
                    properties: ["text": .string()],
                    required: ["text"]
                )) { args, _ in
                    struct Args: Decodable { let text: String }
                    let parsed = try JSONDecoder().decode(Args.self, from: Data(args.utf8))
                    let words = parsed.text.split(separator: " ").count
                    return String(words)
                }
            ]
        )
        ...
        let swarm = Swarm(
            ...
            mode: .sequential,
            members: [tokenAgent, counterAgent]
        )

        let result = try await swarm.run("Analyze this text")

        // Step 1: Real tokenization
        let tokExec = result.memberResults[0].toolExecutions[0]
        #expect(tokExec.toolName == "tokenize")
        #expect(tokExec.succeeded)
        let tokResult = tokExec.result ?? ""
        #expect(tokResult.contains("3") || tokResult.contains("count"), "Should find 3 sentences, got: \(tokResult)")

        // Step 2: Real word counting
        let wcExec = result.memberResults[1].toolExecutions[0]
        #expect(wcExec.toolName == "wordCount")
        // "The quick brown fox jumps. Over the lazy dog. Then it rests." = 12 words when split by space
        #expect(wcExec.result == "12", "Should count 12 words, got: \(wcExec.result ?? "nil")")
    }
    ...
    // MARK: - Combined Skills + Tools in Single Agent

    @Test("Agent with 5 real skills correctly executes them all")
    func testAgentWith5RealSkills() async throws {
        let langCall = ToolCall(id: "tc-1", name: "detectLanguage", arguments: #"{"input": "Hello world"}"#)
        let sentCall = ToolCall(id: "tc-2", name: "analyzeSentiment", arguments: #"{"input": "I love this!"}"#)
        let entCall = ToolCall(id: "tc-3", name: "extractEntities", arguments: #"{"input": "Tim Cook at Apple in Cupertino"}"#)
        let tokCall = ToolCall(id: "tc-4", name: "tokenize", arguments: #"{"input": "One. Two. Three.", "unit": "sentence"}"#)
        let shellCall = ToolCall(id: "tc-5", name: "runShellCommand", arguments: #"{"input": "echo done"}"#)

        let agent = Agent(
            configuration: AgentConfiguration(name: "FullSkillAgent"),
            model: MockProvider(
                responses: ["All 5 skills executed."],
                toolCallResponses: [([langCall, sentCall, entCall, tokCall, shellCall], "")]
            ),
            nativeTools: [
                ...
                TokenizationSkill(),
                ShellSkill(allowedCommands: ["echo"]),
            ]
        )

        let result = try await agent.run("Execute all skills")
        #expect(result.toolExecutions.count == 5)

        // All should succeed
        for exec in result.toolExecutions {
            #expect(exec.succeeded, "Tool \(exec.toolName) failed: \(exec.error ?? "?")")
        }

        // Spot check real results
        let langExec = result.toolExecutions.first(where: { $0.toolName == "detectLanguage" })!
        #expect(langExec.result?.contains("en") == true)

        let shellExec = result.toolExecutions.first(where: { $0.toolName == "runShellCommand" })!
        #expect(shellExec.result?.contains("done") == true)

        let tokExec = result.toolExecutions.first(where: { $0.toolName == "tokenize" })!
        #expect(tokExec.result?.contains("3") == true, "Should tokenize into 3 sentences")
    }
}
